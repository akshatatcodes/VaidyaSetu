...
const AccessDenied = ({ requiredRole = 'doctor', currentRole }) => {
  const navigate = useNavigate();
  const { userRole, logout } = useAuth();
  const activeRole = currentRole || userRole || 'patient';
  ...
  const targetHome = roleHomeMap[activeRole] || '/';
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl p-8 border-2 border-rose-500/30 shadow-2xl text-center space-y-6">
        ...
        <div>
          <span className="px-3 py-1 rounded-full bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-black uppercase tracking-wider">
            Clinical Security Boundary (§34)
          </span>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white mt-2">
            Access Restricted
          </h2>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">
            This route requires <strong>{requiredRole.toUpperCase()}</strong> role authorization. Your current active role is <strong className="text-rose-500 uppercase">{activeRole}</strong>.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-white/10 flex flex-col gap-3">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Return to My Allowed Portal ({activeRole.toUpperCase()})
          </button>
